...
const actions = [{ icon: <SaveIcon />, name: "등록", id: "create" }];
const speechType = ["verb", "noun", "adjust"];
...
export default function Register({ onReceiveData }: RegisterProps) {
  ...
  const dialogClose = async () => {
    if (partOfSpeech === "verb") {
      const ww = Object.values(word).every(
      ...
      Object.values(alphabet).forEach((val) => {
        const { ro = "" } =
          val.find(({ jp = "" }) => jp === japan[japan.length - 1]) || {};
        if (ro) data.endingro = ro;
      });
      data.stemro = roman.endsWith(data.endingro)
        ? roman.slice(0, -data.endingro.length)
        : roman;
      ...
      const { form, exception, example, examplekorean, ...rest } = word;
      const ww = Object.values(rest).every(
        (value) => value !== null && value !== undefined && value !== ""
      );
      if (!ww) return alert("칸을 비우지마세요");
      ...
      try {
        await axios.post("/api/collections/adjust/records", rest);
      } catch (err) {
        return;
      }
    } else {
    ...
    <div>
      ...
      <Dialog onClose={dialogClose} open={open}>
        ...
        <DialogContent>
          ...
          {speechType.includes(partOfSpeech) ? (
            <Box
              component="form"
              sx={{
