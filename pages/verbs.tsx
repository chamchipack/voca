...
import { Card, CardContent, Grid, Typography, Container, Modal, Select, MenuItem } from '@mui/material';
...
const menuItem = [{ text: '했다', value: '6sbhlhlv5fgoyj1' }, { text: '안하다', value: '6qpelflv5fgoyj1' }]
...
const Verbs: React.FC = () => {
    ...
    const handleLogicChange = (event: React.ChangeEvent<{ value: unknown }>) => {
        ...
        const { value: _value } = logic.find(({ id }) => targetId === id)

        const romaji = _value[selectedWord.endingro].split('_')
        let result = ''
        romaji.map(o => {
    ...
    return (
        <Container maxWidth="sm" style={{ marginTop: '20px' }}>
            <Grid container spacing={2}>
                {item.map((word, index) => (
                    <Grid item xs={6} key={index}>
                        <Card variant="outlined" style={{ borderColor: 'green', cursor: 'pointer' }} onClick={() => handleOpen(word)}>
                            <CardContent>
            ...
            >
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 300, background: 'white', padding: '20px', outline: 'none' }}>
